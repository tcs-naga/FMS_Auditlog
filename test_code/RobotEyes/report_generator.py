...
def generate_report(baseline_folder, results_folder, actual_folder=None):
    sleep(1)
    img_path = os.path.join(results_folder, 'visual_images').replace('\\', '/')
    report_path = os.path.join(results_folder, 'output.xml').replace('\\', '/')
    ...
    output_path = os.path.join(os.getcwd(), results_folder, 'visualReport.html').replace('\\', '/')
    logger.debug("Creating visual report at %s" % output_path)
...
def make_test_table(html, baseline_folder, relative_baseline_folder_path, img_path, test_name, folder_name):
    ...
    if not os.path.exists(baseline_folder + os.path.sep + folder_name):
        return  html
    ...
    for filename in os.listdir(baseline_folder + os.path.sep + folder_name):
        if filename.endswith('.png'):
            ...
            if os.path.exists(os.path.join(baseline_folder, folder_name, filename)):
                base_img_path = os.path.join(relative_baseline_folder_path, folder_name, filename).replace('\\', '/')
            ...
            if os.path.exists(os.path.join(img_path, 'actual', folder_name, filename)):
                actual_img_path = os.path.join('visual_images', 'actual', folder_name, filename).replace('\\', '/')
                html += '''<td><a href="%s" target="_blank"><img src="%s" height="200" width="350"></a></td>''' \
            ...
            if os.path.exists(os.path.join(img_path, 'diff', folder_name, filename)):
                diff_img_path = os.path.join('visual_images', 'diff', folder_name, filename).replace('\\', '/')
                html += '''<td><a href="%s" target="_blank"><img src="%s" height="200" width="350"></a></td>''' \
...
def make_non_web_test_table(html, baseline_folder, relative_baseline_folder_path, img_path, test_name,
    ...
    test_folder = os.path.join(img_path, 'actual', folder_name).replace('\\', '/')
    ...
    for filename in os.listdir(test_folder):
        if filename.split('.')[-1] in IMAGE_EXTENSIONS:
            ...
            baseline_img_path = os.path.join(baseline_folder, names_arr[0]).replace('\\', '/')
            actual_img_path = os.path.join(actual_folder, names_arr[1]).replace('\\', '/')
            html += '''<tr>'''
            if os.path.exists(baseline_img_path):
                base_img_path = os.path.join(relative_baseline_folder_path, names_arr[0]).replace('\\', '/')
                html += '''<td><a href="%s" target="_blank"><img src="%s" height="200" width="350"></a></td>''' \
                        % (base_img_path, base_img_path)
            else:
                html += '''<td></td>'''
            ...
            if os.path.exists(actual_img_path):
                actual_img_path = os.path.join(relative_actual_folder_path, names_arr[1]).replace('\\', '/')
                html += '''<td><a href="%s" target="_blank"><img src="%s" height="200" width="350"></a></td>''' \
                        % (actual_img_path, actual_img_path)
            else:
                html += '''<td></td>'''
            ...
            arr = filename.split('.')
            if os.path.exists(os.path.join(img_path, 'actual', folder_name, filename)):
                diff_img_path = os.path.join('visual_images', 'actual', folder_name, filename).replace('\\', '/')
                html += '''<td><a href="%s" target="_blank"><img src="%s" height="200" width="350"></a></td>''' \
                        % (diff_img_path, diff_img_path)
            elif os.path.exists(
                    img_path + os.path.sep + 'actual' + os.path.sep + folder_name + os.path.sep + arr[0] + '-0.png'):
                diff_img_path = str(os.path.join('visual_images', 'actual', folder_name) + os.path.sep + arr[0] + '-0.png').replace('\\', '/')
                html += '''<td><a href="%s" target="_blank"><img src="%s" height="200" width="350"></a></td>''' \
                        % (diff_img_path, diff_img_path)
            else:
                html += '''<td></td>'''
