...
def images(test):
    ...
    baseline_directory = os.path.join(baseline, test).replace('\\', '/')
    actual_directory = os.path.join(results, 'visual_images', 'actual', test).replace('\\', '/')
    diff_directory = os.path.join(results, 'visual_images', 'diff', test).replace('\\', '/')

    if os.path.isdir(actual_directory):
        for file in os.listdir(actual_directory):
            if file.endswith('.png'):
                images = []
                images.append(os.path.join(baseline_directory, file).replace('\\', '/'))
                images.append(os.path.join(actual_directory, file).replace('\\', '/'))
                images.append(os.path.join(diff_directory, file).replace('\\', '/'))

                file = file + '.txt'
                f = os.path.join(actual_directory, file).replace('\\', '/')
...
def report():
    ...
    actual_path = os.path.join(results, 'visual_images', 'actual').replace('\\', '/')
    diff_path = os.path.join(results, 'visual_images', 'diff').replace('\\', '/')
            ...
            if os.path.isdir(abs_directory):
                ...
                for file in os.listdir(abs_directory):
                    if file.endswith('.png'):
                        ...
                        b_img = os.path.join(baseline_path, directory, file).replace('\\', '/')

                        row.append(b_img)
                        row.append(os.path.join(abs_directory, file).replace('\\', '/'))
                        row.append(os.path.join(diff_path, directory, file).replace('\\', '/'))

                        txt_file_name = file + '.txt'
                        txt_file_path = os.path.join(abs_directory, txt_file_name).replace('\\', '/')
...
def make_all_baseline():
    ...
    for test in tests:
        baseline_test = os.path.join(baseline_path, test).replace('\\', '/')
        actual_test = os.path.join(actual_path, test).replace('\\', '/')
...
def baseline_images():
    ...
    for img in imges:
        actual_path = os.path.join(results, img).replace('\\', '/')
