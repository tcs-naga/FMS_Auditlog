...
class RobotEyes(object):
    ...
    def __init__(self, tolerance=0):
        ...
        self.lib = 'none'
        self.force_update_base_images = False

    @property
    def path(self):
        if self.force_update_base_images:
            return os.path.join(self.baseline_dir, self.test_name_folder).replace('\\', '/')
        else:
            return os.path.join(self.images_base_folder, ACTUAL_IMAGE_BASE_FOLDER, self.test_name_folder).replace('\\', '/')

    def log_image_change(self, base_img, actual_img):
        message = f'</td></tr><tr><td colspan="100"><b><span style="background-color: #FFFF00">Image Updated From ====> To</span></b></td></tr><tr><td colspan="100">' \
                  f'<a title="Original image" href="{base_img}" style="padding-right: 20px;"><img title="Original image" src="{base_img}"></a>' \
                  f'<a title="Updated image" href="{actual_img}"><img title="Updated image" src="{actual_img}"></a>'
        logger.info(message, True, False)

    def log_image_compare(self, base_img, actual_img, diff):
        message = f'</td></tr><tr><td colspan="100"><b><span style="background-color: #FFFF00">Image compared From ====> To</span></b></td></tr><tr><td colspan="100">' \
                  f'<a title="Original image" href="{base_img}" style="padding-right: 20px;"><img title="Original image" src="{base_img}"></a>' \
                  f'<a title="Updated image" href="{actual_img}"><img title="Updated image" src="{actual_img}"></a>' \
                  f'<a title="Updated image" href="{diff}"><img title="Updated image" src="{diff}"></a>'
        logger.info(message, True, False)

    def open_eyes(self, lib='SeleniumLibrary', tolerance=0, template_id='', cleanup=None, force_update_base_images=False, start_port=None):
        ...
        self.lib = lib
        self.images_base_folder = os.path.join(self.output_dir, IMAGES_FOLDER).replace('\\', '/')
        self.force_update_base_images = force_update_base_images
        if lib.lower() != 'none':
        ...
        logger.debug("roboteyestestfolder: %s" % self.test_name_folder)
        # delete if directory already exist. Fresh test
        self._delete_folder_if_exists()
        self.count = 1

    # Captures full screen
    def capture_full_screen(self, tolerance=None, blur=[], radius=50, name=None, redact=[], redact_region=[]):
        ...
        self.browser.capture_full_screen(path, blur, radius, redact, redact_region)
    ...
    # Captures full screen
    def capture_full_screen_and_compare(self, tolerance=None, blur=[], radius=50, name=None, redact=[], redact_region=[], retry: int=5):
        # recreate deleted folder
        self._create_empty_folder()
        tolerance = float(tolerance) if tolerance else self.tolerance
        tolerance = tolerance/100 if tolerance >= 1 else tolerance
        ...
        name += '.png'
        path = os.path.join(self.path, name).replace('\\', '/')
        # Before we capture image, ensure the webpage is stable.
        if self.force_update_base_images:
            BuiltIn().run_keyword("Wait for user input")
        old_log_level = BuiltIn().run_keyword('Set Log Level', 'INFO')
        self.browser.capture_full_screen(path, blur, radius, redact, redact_region)
        BuiltIn().run_keyword('Set Log Level', old_log_level)
        self._fix_base_image_size(path, name)
        self.stats[name] = tolerance
        self.count += 1
        if self.force_update_base_images or not self.compare_image(name):
            return True
        else:
            while(retry >= 0):
                time.sleep(1)
                ...
                self._fix_base_image_size(path, name)
                self.stats[name] = tolerance
                if not self.compare_image(name):
                    return True
                retry -= 1
        BuiltIn().run_keyword('Fail', 'Image dissimilarity exceeds tolerance {}'.format(str(tolerance)))
    ...
    def capture_element(self, selector, tolerance=None, blur=[], radius=50, name=None, redact=[], redact_region=[]):
        # recreate deleted folder
        self._create_empty_folder()
        tolerance = float(tolerance) if tolerance else self.tolerance
        tolerance = tolerance/100 if tolerance >= 1 else tolerance
        ...
        path = os.path.join(self.path, name).replace('\\', '/')
        time.sleep(1)
        self.browser.capture_element(path, selector, blur, radius, redact, redact_region)
        self._fix_base_image_size(path, name)
        self.stats[name] = tolerance
        self.count += 1

    def capture_given_element(self, element, name=None, tolerance=None, blur=[], radius=50, redact=[], redact_region=[]):
        # recreate deleted folder
        self._create_empty_folder()
        tolerance = float(tolerance) if tolerance else self.tolerance
        tolerance = tolerance/100 if tolerance >= 1 else tolerance
        ...
        path = os.path.join(self.path, name).replace('\\', '/')
        time.sleep(1)
        self.browser.capture_given_element(path, element=element, blur=blur, radius=radius, redact=redact, redact_region=redact_region)
        self._fix_base_image_size(path, name)
        self.stats[name] = tolerance
        self.count += 1

    def capture_given_element_and_compare(self, element, name=None, tolerance=None, blur=[], radius=50, redact=[], redact_region=[], retry: int=5):
        # recreate deleted folder
        self._create_empty_folder()
        tolerance = float(tolerance) if tolerance else self.tolerance
        tolerance = tolerance/100 if tolerance >= 1 else tolerance
    ...
    def compare_two_images(self, ref, actual, output, tolerance=None):
        ...
        first_path = os.path.join(self.baseline_dir, ref).replace('\\', '/')
        second_path = os.path.join(self.actual_dir, actual).replace('\\', '/')

        if os.path.exists(first_path) and os.path.exists(second_path):
            diff_path = os.path.join(self.path, output).replace('\\', '/')
            ...
            txt_path = os.path.join(self.path, output + '.txt').replace('\\', '/')
    ...
    def compare_images(self):
        ...
        baseline_path = os.path.join(self.baseline_dir, test_name).replace('\\', '/')
        actual_path = os.path.join(self.images_base_folder, ACTUAL_IMAGE_BASE_FOLDER, test_name).replace('\\', '/')
        diff_path = os.path.join(self.images_base_folder, DIFF_IMAGE_BASE_FOLDER, test_name).replace('\\', '/')
        ...
        for filename in os.listdir(actual_path):
            if filename.endswith('.png'):
                b_path = os.path.join(baseline_path, filename).replace('\\', '/')
                a_path = os.path.join(actual_path, filename).replace('\\', '/')
                d_path = os.path.join(diff_path, filename).replace('\\', '/')
                txt_path = os.path.join(actual_path, filename + '.txt').replace('\\', '/')
        ...
        BuiltIn().run_keyword('Fail', 'Image dissimilarity {} exceeds threshold {}'.format(str(difference), str(threshold))) if self.fail else ''

    def compare_image(self, image_name: str):
        """
        Return False if fails to compare image.
        """
        image_name += '.png' if image_name.split('.')[-1] not in IMAGE_EXTENSIONS else ''
        test_name = self.test_name.replace(' ', '_')
        baseline_path = os.path.join(self.baseline_dir, test_name).replace('\\', '/')
        actual_path = os.path.join(self.images_base_folder, ACTUAL_IMAGE_BASE_FOLDER, test_name).replace('\\', '/')
        diff_path = os.path.join(self.images_base_folder, DIFF_IMAGE_BASE_FOLDER, test_name).replace('\\', '/')
        ...
        # compare actual and baseline images and save the diff image
        b_path = os.path.join(baseline_path, image_name).replace('\\', '/')
        a_path = os.path.join(actual_path, image_name).replace('\\', '/')
        d_path = os.path.join(diff_path, image_name).replace('\\', '/')
        txt_path = os.path.join(actual_path, image_name + '.txt').replace('\\', '/')

        if os.path.exists(b_path):
            difference = Imagemagick(b_path, a_path, d_path).compare_images()
            try:
                threshold = float(self.stats[image_name])
            except ValueError:
                raise Exception('Invalid tolerance: %s' % self.stats[image_name])
        ...
        if color == self.pass_color and self.cleanup_files is not None:
            ...
            b_path = os.path.relpath(b_path, self.output_dir).replace('\\', '/')
            a_path = os.path.relpath(a_path, self.output_dir).replace('\\', '/')
            d_path = os.path.relpath(d_path, self.output_dir).replace('\\', '/')
            self.log_image_compare(b_path, a_path, d_path)
        return self.fail
    ...
    def _delete_folder_if_exists(self):
        actual_image_test_folder = os.path.join(self.images_base_folder, ACTUAL_IMAGE_BASE_FOLDER, self.test_name_folder).replace('\\', '/')
        diff_image_test_folder = os.path.join(self.images_base_folder, DIFF_IMAGE_BASE_FOLDER, self.test_name_folder).replace('\\', '/')
    ...
    def _create_empty_folder(self):
        if self.lib.lower() != 'none':
            self._path = os.path.join(self.baseline_dir, self.test_name_folder).replace('\\', '/')

            if not os.path.exists(self._path):
                os.makedirs(self._path)

        self._path = os.path.join(self.images_base_folder, ACTUAL_IMAGE_BASE_FOLDER, self.test_name_folder).replace('\\', '/')
    ...
    def _fix_base_image_size(self, path, image_name):
        test_name = self.test_name.replace(' ', '_')
        base_image = os.path.join(self.baseline_dir, test_name, image_name).replace('\\', '/')
    ...
    def _close(self):
        if self.baseline_dir and self.output_dir:
            ...
            # If port is defined, the baseline directory is 2 level up.
            if self.start_port:
                path = os.path.join(self.output_dir, '..', '..')
            base_dir = os.path.relpath(self.baseline_dir, path).replace('\\', '/')
            thread = Thread(
                target=generate_report,
                args=(base_dir, self.output_dir, self.actual_dir,)
            )
            thread.start()
