__copyright__ = "https://github.com/peterservice-rnd/robotframework-rabbitmq/tree/master/src"
...
from test_code.Const import RABBITMQ_POLL_THRESHOLD_SECS
...
class RabbitMq(object):
    ...
    def create_queue(self, queue_name: str = None, auto_delete: bool = False, durable: bool = False,
                     node: str = None, arguments: Dict[str, Any] = None) -> str:
        """
        Create queue.
        ...
        *Returns:*\n
        ...
        """
        if queue_name == None:
            result = self._get_channel().queue_declare(queue='', exclusive=True)
            queue_name = result.method.queue
            logger.debug(f'Created random queue: {queue_name}')
        else:
    ...
    def connect_to_rabbitmq_exchange(self, exchange_name: str='event', routing_key: str='#', queue_name: str='ATFQ') -> None:
        """
        Connect to rabbitmq exchange, create a queue for test and bind all messages from
        exchange and listen to routing key if passed or all messages.

        *Args:*
            exchange_name: exchange name (default=event)
            routing_key: routing key (default=None)
            queue_name: queue_name; (default=ATFQ)

        *Example:*\n
        | Connect To Rabbitmq Exchange | event | asset.telemetry.report.${DEFAULT_ASSET_ID} | ATFQ |
        """
        # Connect to local host rabbitMq using default ports and credentials
        imperium_services = BuiltIn().get_library_instance('test_code.ImperiumServices')
        rq = imperium_services.rabbitmq
        self.create_rabbitmq_connection(host=rq['host'], http_port=rq['manager_port'], amqp_port=rq['port'], username=rq['user'], password=rq['password'], alias='rmq', vhost='/')
        logger.debug(self.overview())

        # Create queue
        queue_name = self.create_queue(queue_name)

        #Bind exchange with new queue
        self.binding_exchange_with_queue(exchange_name, queue_name, routing_key)


    def get_message_from_rabbitmq(self, queue_name: str='ATFQ', ack: bool=True) -> RabbitMqMessage:
        """
            Gets first message from the queue.
            get_message_from_queue (for Getting single message from RabbitMQ queue)
            for the duration specified in RABBITMQ_POLL_THRESHOLD_SECS
        ...
        *Returns:*\n
            body: message body.
            If queue is still empty after RABBITMQ_POLL_THRESHOLD_SECS, returns None, None, None.

        *Example:*\n
        | Get Message From Rabbitmq | queue_name | ATFQ |
        """
        for x in range(RABBITMQ_POLL_THRESHOLD_SECS - 1):
            sleep(1)
            _, _, body = self.get_message_from_queue(queue_name=queue_name, ack=ack)
            if body != None:
                break
        if not body:
            raise Exception(f'Failed to find the message in the queue: {queue_name} after {RABBITMQ_POLL_THRESHOLD_SECS} seconds')
